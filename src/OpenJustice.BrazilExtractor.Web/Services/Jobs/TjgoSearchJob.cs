...
/// Orchestrates TJGO search + PDF download and enforces query-level cadence.
/// OCR is intentionally manual via the OCR Only workflow.
/// </summary>
public class TjgoSearchJob : ITjgoSearchJob
{
    ...
    public async Task<TjgoSearchResult> ExecuteAsync(DateTime? queryDate = null, CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Starting TJGO search job execution");

        // Determine the query date - explicit override from caller takes precedence.
        var effectiveQueryDate = (queryDate ?? _options.QueryDateWindowStartDate ?? DateTime.Today).Date;
        ...
        var query = TjgoSearchQuery.ForSingleDay(effectiveQueryDate, _options.CriminalMode);

        var queryStartTime = DateTime.UtcNow;
        
        _logger.LogInformation(
            "Executing search for date {Date} (formatted: {FormattedDate}), Criminal mode: {CriminalMode}",
            effectiveQueryDate.ToString("yyyy-MM-dd"),
            query.FormattedDate,
            query.CriminalMode);
        ...
        if (result.Success)
        {
            ...
            if (result.PdfLinks.Count > 0)
            {
                _logger.LogInformation(
                    ...
                    result.PdfLinks.Count, effectiveQueryDate.ToString("yyyy-MM-dd"));
                ...
                if (downloadResult.SucceededCount > 0)
                {
                    _logger.LogInformation("Extraction stage complete. OCR is manual via OCR Only workflow.");
                }
                else
                {
                    _logger.LogInformation("No files downloaded");
                }
            }
            else
            {
    ...
    private void LogAcquisitionTelemetry(TjgoSearchResult result)
    {
        ...
        _logger.LogInformation("OCR: Manual workflow (use OCR Only button)");
