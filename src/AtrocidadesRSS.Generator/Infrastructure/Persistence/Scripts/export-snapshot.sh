...
OUTPUT_FILE="${1:-atrocidadesrss_v1_$(date +%Y%m%d_%H%M%S).sql}"
...
dotnet ef migrations script \
    --idempotent \
    --output "$OUTPUT_FILE" \
    --project "$PROJECT_DIR/AtrocidadesRSS.Generator.csproj" \
    --startup-project "$PROJECT_DIR/AtrocidadesRSS.Generator.csproj" \
    2>/dev/null || {
