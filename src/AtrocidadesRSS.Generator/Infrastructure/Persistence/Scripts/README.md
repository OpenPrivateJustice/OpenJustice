...
```bash
cd src/AtrocidadesRSS.Generator
dotnet ef database update --project src/AtrocidadesRSS.Generator --startup-project src/AtrocidadesRSS.Generator
```
...
```bash
cd src/AtrocidadesRSS.Generator
dotnet ef database update 0 --project src/AtrocidadesRSS.Generator --startup-project src/AtrocidadesRSS.Generator
```
...
```bash
dotnet ef migrations list --project src/AtrocidadesRSS.Generator --startup-project src/AtrocidadesRSS.Generator
```

### Generate SQL Script from Migration

```bash
dotnet ef migrations script --project src/AtrocidadesRSS.Generator --startup-project src/AtrocidadesRSS.Generator --output ./Scripts/up.sql
```
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Database=atrocidadesrss;Username=postgres;Password=your_password"
  }
}
```
...
pg_dump -h localhost -U postgres -d atrocidadesrss -f backup_$(date +%Y%m%d).sql
```
...
psql -h localhost -U postgres -c "DROP DATABASE IF EXISTS atrocidadesrss;"
psql -h localhost -U postgres -c "CREATE DATABASE atrocidadesrss;"

# Restore from backup file
psql -h localhost -U postgres -d atrocidadesrss -f backup_20260301.sql
```
...
```bash
dotnet ef migrations list --project src/AtrocidadesRSS.Generator
```
